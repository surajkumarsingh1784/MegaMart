...
const Cart = () => {
    const {Products,Currency, cartItems, RemoveFromCart, getCartCount, updateCartItem, getCartAmount, axios, user, setCartItems} = useAppContext();
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={()=>{
                                navigate(`/products/${Array.isArray(product.category) ? product.category[0].toLowerCase() : product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)
                            }} className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded">
                                {product.images && product.images[0] ? (
                                    <img className="max-w-full h-full object-cover" src={product.images[0]} alt={product.name} />
                                ) : (
                                    <img className="max-w-full h-full object-cover" src={assets.fallback_image || assets.cart_icon} alt="No image" />
                                )}
                            </div>
                            <div>
